...
- [XTEA](#XTEA)
- [Генератора ключевой гаммы на основе Chacha20](#Генератор-ключевой-гаммы-на-основе-chacha20)
...
### Генератор ключевой гаммы на основе chacha20
___

```
encrypter chacha {режим} [опции]: 
```

Данный генератор будет полезен для генерации псевдослучайной последовательности очень большой длины (период до 2^70). В теории, если объединить такой ключ с шифром цезаря, то можно получить абсолютно криптостойкий шифр (шифр Вернера) для файлов размером не более указанного периода.

| Режимы | Описание |
|---|---|
| `gen` |  Сгенерировать ключевую гамму по файлу-ключу. Файл-ключ должен содержать последовательность из 16 байт, желательно, случайно сгенерированную. Опции `-size` и `-kf` обязательны. |

| Опции|Описание|
|---|---|
| `-kf "/path/to/file.key"`    | Использование ключ-файла по заданному пути |
| `-of "/path/to/gamma"` | Вывод гаммы в файл |
| `-size 1048572` или `-s 1048572`| Длина генерируемой ключевой гаммы в байтах |

Справку можно получить по команде 
```
encrypter chacha -?
```
Для _генерации_ файла-гаммы на 512 Мб выполните:
```
encrypter chacha gen -kf "/path/to/file.key" -of "/path/to/gamma" -size 536870912
```

## Редактирование конфигурационного файла

На данный момент доступно:
- переключение языка сообщений программы (русский/английский). 

Для этого достаточно отредактировать файл `config.cfg`. Его возможно редактировать _вручную_ так и через команду. Более подробное описание редактирования доступно по: 
```
encrypter config -?
```
