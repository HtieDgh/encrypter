...
```
Если не используется операторы перенаправления потока `<` и `>`, то шифрует/дешифрует ввод с клавиатуры и выводит результат в консоль. Некоторые алгоритмы имееют опции конфликтующие с `<` и `>`, поэтому использование их возможно, но не рекомендуется. Также замечано неопределенное поведение из за особеностей обработки текстовых/бинарных файлов стандартными потоками ввода/вывода в разных операционных системах. Более релевантные примеры использования программы представлены ниже.
