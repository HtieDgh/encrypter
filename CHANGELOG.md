...
### Added
- Добавлен включаемый режим интерпретатора команд: вместо перезапуска всей программы достаточно вводить команды выбранного алгоритма. Удобно когда нужно много раз шифровать/дешифровать разные файлы одним алгоритмом.
- Добавлен генератор ключевой гаммы на основе chacha20, команда `encrypter chacha -help`, генерирующий псевдослучайную последовательность произвольной длинны из 32-байтного ключа.  
- Добавлена опция `-kf` для шифра _Цезаря_. Она позволяет определить файл-ключ, байты из которого будут использованы для операций шифрования / дешифрования

### Fixed
- Обращение по _nullptr_ в `controller->doAlgorithm()` в случае вызова справки, когда алгоритм шифрования определить не удалось.

### Refactor
- Изменено имя класса `AlgorithmStrategy` и соответствующих файлов  на `Controller`.
- Перевод точки входа на `wmain`, смена порядка обработки в классах реализаций алгоритмов.  
